<?php
$display_phone_no = $organizer->phone_no;
// if it's more than 10 characters, then it must have +63
// so lets trim the number so it can be displayed in the form
if (strlen($organizer->phone_no) > 10) {
    $display_phone_no = substr($organizer->phone_no, 3);
}
?>

<x-layout title="Edit {{$organizer->name}}" :show_navbar="true" :show_heading="true">
    <x-slot:heading>
        Edit Organization: {{ $organizer->name }}
    </x-slot:heading>
    <form class="p-6 lg:p-8" method="POST" action="/organizers/{{$organizer->id}}">
        @csrf
        @method('PATCH')
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-4  max-w-xs">
                        ...
                        <div class="join mt-2 w-full">
                            ...
                            <input id="phone_no" name="phone_no" type="number" placeholder="912 345 6789" required
                                   value="{{ old('phone_no') ?? $display_phone_no }}"
                                   minlength="10"
                                   maxlength="10"
                    ...
                    <div class="col-span-full">
                        ...
                        <textarea id="description" name="description"
                                  ...
                                  placeholder="Bio">{{ old('description') ?? $organizer->description }}</textarea>
        ...
        <div class="mt-6 flex items-center justify-between">
            <button class="btn btn-error" href="/organizers/{{$organizer->id}}/delete" form="delete">
                Delete
            </button>
            <div class="flex gap-x-6">
                <a class="btn" href="/organizers/{{$organizer->id}}">Cancel</a>
                <button type="submit" class="btn btn-primary">Update</button>
            </div>
        </div>
    </form>
    <form id="delete" method="POST" action="/organizers/{{$organizer->id}}" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</x-layout>
